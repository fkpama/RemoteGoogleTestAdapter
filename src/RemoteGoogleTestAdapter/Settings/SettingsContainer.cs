...
using GTAUtils = GoogleTestAdapter.Helpers.Utils;
    ...
    public class RunSettingsContainer : TestRunSettings, IGoogleTestAdapterSettingsContainer
    {
        ...
        public static RunSettingsContainer LoadFromXml(XPathNavigator rootElement)
        {
            ...
            if (rootElement.Name.Equals(GoogleTestConstants.SettingsName))
            {
                ...
                var schemaStream = new StringReader(Resources.GoogleTestAdapterSettings);
                ...
                try
                {
                    ...
                    if (serializationContainer is not null)
                    {
        ...
        {
            ValidateOne("ProjectRegex", settings.ProjectRegex, GTAUtils.ValidateRegex);
            ValidateOne("TestDiscoveryRegex", settings.TestDiscoveryRegex, GTAUtils.ValidateRegex);
            ValidateOne("TraitsRegexesBefore", settings.TraitsRegexesBefore, GTAUtils.ValidateTraitRegexes);
            ValidateOne("TraitsRegexesAfter", settings.TraitsRegexesAfter, GTAUtils.ValidateTraitRegexes);
